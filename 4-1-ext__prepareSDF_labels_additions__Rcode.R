...
# IF ON SKIN:   	-> IF ON SKIN,
# IF INHALED:   	-> IF INHALED,
# IF IN EYES:   	-> IF IN EYES,
# IF ON CLOTHING:	-> IF ON CLOTHING,
# concerned: 		-> concerned,
# occurs: 			-> occurs,
# persists:			-> persists,
# symptoms: 		-> symptoms,
...
# Input
# Compounds
sdf_raw <- read_file("C:/.../PubChem_compound_cache_rukLgqoMz7D4nkeHxf8OoA5QnDBosCu-UZsw8kqKIvNKkx4_records_SD.SDF") |>
				#str_replace_all("\n", "\r\n") |>
				str_trim() |>
				as_tibble() |>
...
mna_cids <- sdf_raw |> select(mna_cid, all_cid) |>
					   ...
					   distinct()
# Read the data
data_raw <- read_tsv("C:/.../env_precautions_cid.tsv") |>
					mutate(cid = as.character(cid)) |>
					select(-hnid) |>
					mutate(warning = str_replace(warning, coll("SWALLOWED:"), "IF SWALLOWED,")) |>
					mutate(warning = str_replace(warning, coll("SKIN:"), "SKIN,")) |>
					mutate(warning = str_replace(warning, coll("INHALED:"), "INHALED,")) |>
					mutate(warning = str_replace(warning, coll("EYES:"), "EYES,")) |>
					mutate(warning = str_replace(warning, coll("CLOTHING:"), "CLOTHING,")) |>
					mutate(warning = str_replace(warning, coll("concerned:"), "concerned,")) |>
					mutate(warning = str_replace(warning, coll("occurs:"), "occurs,")) |>
					mutate(warning = str_replace(warning, coll("persists:"), "persists,")) |>
					mutate(warning = str_replace(warning, coll("symptoms:"), "symptoms,")) |>
					mutate(warning = str_replace(warning, coll("fire:"), "fire,")) |>
					mutate(warning = str_replace(warning, coll("quantities:"), "quantities,")) |>
					mutate(warning = str_replace(warning, coll("):"), "),")) |>
					mutate(warning = str_replace(warning, coll("50 ?C/122?F"), "50 C")) |>
					mutate(warning = str_replace(warning, coll("?C?F"), "")) |> 
...
pictogram <- read_tsv("C:/.../GHS_picts_additions.tsv") |>
					mutate(hazard_label_pict = case_when(
							Pictogram == "GHS07" ~ "irritant",
							Pictogram == "GHS09" ~ "environment hazard"
						))
# Summary
...
## Export SDFs
write_lines(str_c("", sdf_hlp[[1]]), "C:/.../sdfs/hazard_labels_PC_CIDP_NLM__ghs_extended.SDF")
