...
mna_cids <- sdf_raw |> select(mna_cid, all_cid) |>
					   ...
					   distinct()
# Read the data
data_raw <- read_tsv("C:/.../data/env_precautions_cid.tsv") |>
...
# Leave H-codes only
data_h <- data_main |> filter(str_starts(code, "H"))

# Prepare the data
# Codes and all
data <- data_h |> group_by(mna_cid) |>
...
## Export SDFs
write_lines(str_c("", sdf[[1]]), "C:/.../data/sdfs/hazard_labels_PC_CIDP_NLM__ghs_extended_detailed.SDF")
